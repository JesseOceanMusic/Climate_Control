// update v050 - Settings > Stack Protection Enable //

///↓↓↓ ОТЛАДКА ↓↓↓///
//#define local_DEBUG


// большинство переменных хранятся в global, поскульку вначале в одних функциях присваиваются значениям, а после в других функциях как-то используются.
// например вначале приисходит опрос датчиков через функцию "TEMP_request", после в функции "TEMP_test_errors" проверяются значения на нахождение температур в заданных границах.
// и лишь после этого можно сформировать сообщение для телеграма или для синхронизации двух форматов через функции "TEMP_text_output" и "TEMP_excel_output".

// есть какое-то количество переменных, что записаны в global для удоства чтения кода. поскольку программа готова и код помещается в память esp, нет смысла жертвовать удобством ради экономии памяти, которая уже не нужна.

// по совету Артёма я попытался реализовать функцию обработки сообщений через switch, но код стал нечитаемым из-за того, что пришлось бы разнести первую часть сообщения и вторую в разные части кода, что было бы крайне неудобно.
// к тому же, я проверил сколько занимает проверки всех else if подряд. оказалось, что обработка всех else if занимает меньше 1 милисекунды.
// в то же время, отправка сообщения в телеграм занимает 1 секунду (что в 1000 раз дольше). В виду этого - оптимизация обработки сообщений через замену else if на switch считаю нецелесообразной.
// UPDATE - частично switch в обработке сообщений я всё же реализовал.
// UPDATE.2 - обработка сообщений реализована полностью на switch.
...
// для флагов используется int вместо bool для того, чтобы было проще и быстрее добавлять дополнительные состояния при необходимости.
...
// Используемые компоненты на 2024-07-26 //
...
// Если буду добавлять датчик со2, то вероятно это будет SCD40/SCD41 на шине i2c.

// на всякий случай версии библиотек:
